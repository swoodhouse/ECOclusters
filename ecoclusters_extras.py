...
def build_hlacocluster_mask(metas: pd.DataFrame, hlacocluster_names: List[str]) -> List[npt.NDArray[np.int_]]:
    """
    Returns a sample by hlacocluster mask. 1 means this sample has the HLA corresponding to this HLA-COcluster, 0
    means it does not.
    ...
    Args:
        ...
        hlacocluster_names (ty.List[str]): list of HLA-COclusters, in 'feature0_A_01_01' format. HLA for HLA-COcluster is
            extracted from its name.

    Returns:
        ty.List[npt.NDArray[np.int_]]: len(hlacoclusters) list of len(metas) numpy array of int.
    """
    # this code is brittle. and also slow. nested for loops in python is not a good idea
    logger.info("build_hlacocluster_mask")
    # hlas = set([t[t.index('_')+1:] for t in hlacocluster_names])
    for hlacocluster in hlacocluster_names:
        if "+" in hlacocluster or ":" in hlacocluster or "*" in hlacocluster or "-" not in hlacocluster:
            raise ValueError("HLA-COclusters have incorrect format!")

    hlas = set([t.split("-")[1] for t in hlacocluster_names])
    logger.info(hlas)
...
class EcoclusterConstructor:
    ...
    def cluster_ecoclusters(
        ...
        hlacocluster_names_in_order: ty.List[str],
        pdf_hlacocluster_tcrs: pd.DataFrame,
    ) -> pd.DataFrame:
        """
        ...
        hlacoclusters_in_clusters: ty.Set[str] = set()

        # make for easy lookup of HLA-COcluster HLAs
        pdf_hlacocluster_hlas = pdf_hlacocluster_tcrs.drop_duplicates("cluster")
        hlacocluster_hla_map = dict(zip(*[pdf_hlacocluster_hlas.cluster, pdf_hlacocluster_hlas.hla]))

        def lookup_hlacoclusterlist_hlas(hlacocluster_list: ty.List[str]) -> ty.Set[str]:
            return set([hlacocluster_hla_map[hlacocluster] for hlacocluster in hlacocluster_list])

        pdf_cluster_tree_distance_desc["n_hlacoclusters"] = (
        ...
        pdf_cluster_tree_distance_desc["hlas"] = (
            pdf_cluster_tree_distance_desc.cluster_members.apply(lookup_hlacoclusterlist_hlas)
        )
        ...
        logger.debug(
            f"    Clusters: {len(cluster_hlacocluster_members)}. Retained {len(hlacoclusters_in_clusters)} "
    ...
    def reformat_ecoclusters_one_row_per_tcr(
        self, pdf_ecoclustering: pd.DataFrame, pdf_hlacocluster_tcrs: pd.DataFrame
    ) -> pd.DataFrame:
...
def report_clusters_entiretree(
    pdf_hlacocluster_node_connectivity: pd.DataFrame,
    hlacocluster_names_in_order: ty.List[str],
    pdf_hlacocluster_tcrs: pd.DataFrame,
) -> pd.DataFrame:
    """
